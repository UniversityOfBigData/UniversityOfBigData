...
1. ユーザー名 `admin` と[Dockerイメージ作成時](README.md#インスタンスの立ち上げ)に `DJANGO_SUPERUSER_PASSWORD` で指定したパスワードを入力しログインします。<br>
...
### チーム割り当て
TAまたは管理者ユーザのメニューの「チーム作成ページ」で 'Name:' の欄にチーム名を入力し「作成」ボタンを押すとチームを作成することができます。

ユーザをチームに割り当てるには、「チーム割付けページ」を選択し、ユーザごとに割り当てるチームをチェックして更新ボタンを押下します。
...
![img/op_003_00.png](img/op_003_00.png)<br>
...
注意点として、コンペ作成手続きを行っても必ずしもすぐには状態が反映されず、1時間毎にコンペティション情報が最新の状態に更新されます。

> コンペティションの状態更新頻度はデフォルトでは1時間ですが、`entrypoint.sh` を編集することで変更が可能です。
> 
...
> `--hour "*/2"` とすると2時間ごと、`--hour "5-7"` とすると5時～7時など[UNIX Cronの書式](https://ja.wikipedia.org/wiki/Cron#%E5%BC%8F)で記述することができます。値はダブルクォーテーション `""` で囲むようにしてください。編集後、コンテナを再起動してください。

コンペティション作成により、トップページ、コンペティション一覧ページ、管理ログ、コンペティションページが更新されます。
![img/op_003.png](img/op_003.png)

### ログの確認
ユーザーの活動やコンペ進行のログを確認することができます。管理者、スタッフ権限で利用可能です。

![img/ma_01.png](img/ma_01.png)<br>

1. ログリストではユーザーのアクセス記録を確認できます。<br>
![img/ma_02.png](img/ma_02.png)<br>
2. 管理ログリストでは、コンペティションの開始、終了の記録を確認できます。<br>
![img/ma_03.png](img/ma_03.png)<br>
