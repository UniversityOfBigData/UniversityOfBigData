...
from apscheduler.schedulers.background import BackgroundScheduler  # 定期実行処理用
...
from universityofbigdata.lib.managementlogger import set_management_log_box
...
def setplan01():
    ...
    for comp in compALL:
        ...
        if (comp.status != status):
            # 変更があった場合、ログ出力
            # print(comp.status, status)
            message = set_management_log_box(word, comp.id)
            logger.info(message)
