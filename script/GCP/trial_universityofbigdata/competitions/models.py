...
METRICS = [
    ('mean_absolute_error', _('平均絶対誤差')),
    ('mean_squared_error', _('二乗平均誤差')),
    ('accuracy', _('正解率')),
    ('recall', _('適合率')),
    ('precision', _('再現率')),
    ('f1', _('f値')),
    ('root_mean_squared_error', _('二乗平均平方根誤差',)),
    ('mean_roc_auc_score', _('Area under the ROC curve (AUC)')),
    # ('roc_auc_score', _('マクロ平均ROC-AUC')),
    ]
...
def savePredictionPath(instance, filename):
    ...
    return f'competition/{competition_name}/data/{new_name}.{ext}'
...
def saveSubmissionPath(instance, filename):
    ext = filename.split('.')[-1]
    competition_name = instance.post.title
    ...
    #print('post :',date_text)
    post_count=instance.count_par_today # 同一チームで本日何個投稿しているか？のカウント
    return f'competition/{competition_name}/{date_text}/{new_name}_{post_count}.{ext}'
...
class CompetitionModel(models.Model):
    ...
    choices_list = METRICS
    evaluation_type = models.CharField(
            _('評価指標'), max_length=50, default='', choices=choices_list)
    ...
    n_posts = models.IntegerField(_('承認数'), default=0)  # 投稿数
    # 参加認証
...
class CompetitionPost(models.Model):
    ...
    intermediate_score = models.DecimalField(
            _('中間スコア'), max_digits=15, decimal_places=3, default=0.0)
    final_score = models.DecimalField(
            _('最終スコア'), max_digits=15, decimal_places=3, default=0.0)
